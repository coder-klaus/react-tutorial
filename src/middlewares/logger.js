// state 是 storeApi 对象 => 是一个只包含了 getState 和 dispatch 的对象
...
export default state => next => action => {
  ...
  if (import.meta.env.DEV) {
    ...
    console.groupCollapsed('[logger] ' + (action.type ?? 'non-plain action'))
    console.log('旧的state', state.getState())
  ...
  // 仅在开发环境打印新的state
  if (import.meta.env.DEV) {
    console.log('新的state', state.getState())
    console.groupEnd()
  }
